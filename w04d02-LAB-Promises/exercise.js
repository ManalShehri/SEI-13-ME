...
addToMenu(newBurger).then(data => {
  ...
  {
    // console.log('before then ',isResolved)
    order(ele.order).then(data => {
      console.log(data)
      isResolved = true;
      // console.log('after then ',isResolved)
    }).catch(erro => {
      console.log(error)
      isResolved = false;
    })
    console.log(ele)
    isResolved = true;
    // console.log('out of then ',isResolved)
  ...
  {
    console.log('some error occured, not all food delivered')
  }